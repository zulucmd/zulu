...
func TestDefaultCompletionCmd(t *testing.T) {
	...
	// Test that when there are no sub-commands, the completion command is not created
	// if it is not called directly
	...
	// Test that when there are no sub-commands, the completion command is created
	// when it is called directly
	_, err := executeCommand(rootCmd, zulu.CompCmdName)
	testutil.AssertNilf(t, err, "Unexpected error: %v", err)
...
func TestCompletionForOneRequiredFlags(t *testing.T) {
	...
	testcases := []struct {
		...
		{
			desc: "flags in one required group suggested without the - prefix",
			args: []string{"child", ""},
			expectedOutput: strings.Join([]string{
				"--ingroup1",
				"--ingroup2",
		...
		{
			desc: "flags in one required group suggested with the - prefix",
			args: []string{"child", "-"},
			expectedOutput: strings.Join([]string{
				"--ingroup1",
				"--ingroup2",
		...
		{
			desc: "when one flag in group is set, other flags in group not suggested",
			args: []string{"child", "--ingroup2", "value", ""},
			expectedOutput: strings.Join([]string{
				"subArg",
				":4",
		...
		{
			desc: "when all flags in group are set, flags not suggested",
			args: []string{"child", "--ingroup1", "8", "--ingroup2", "value2", "--ingroup3", ""},
			expectedOutput: strings.Join([]string{
				"subArg",
				":4",
...
func TestCompletionZuluFlags(t *testing.T) {
	getCmd := func() *zulu.Command {
		rootCmd := &zulu.Command{
			Use:     "root",
			Version: "1.1.1",
...
func TestInitDefaultCompletionCmd(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.desc, func(t *testing.T) {
			...
			if len(tc.args) > 0 && tc.args[0] == zulu.ShellCompRequestCmd {
				expectedNumSubCommands++
			}
...
func TestArgsNotDetectedAsFlagsCompletionInGo(t *testing.T) {
	...
	root := zulu.Command{
		Use: "root",
		ValidArgsFunction: func(cmd *zulu.Command, args []string, toComplete string) ([]string, zulu.ShellCompDirective) {
			return []string{"service", "1-123", "11-123"}, zulu.ShellCompDirectiveNoFileComp
		},
	}
