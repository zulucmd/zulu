...
# NAME
`, header.Title, header.Section, date, header.Source, header.Manual))
...
func manPrintFlags(buf io.StringWriter, flags *zflag.FlagSet) {
	flags.VisitAll(func(flag *zflag.Flag) {
		...
		if hasShorthand { //nolint:nestif // todo refactor later
			format += "**-%c**"
		...
		if !hasShorthand || !flag.ShorthandOnly { //nolint:nestif // todo refactor later
			if isBoolean {
				if flag.AddNegative {
					format += ", **--[no-]%s**"
				} else {
