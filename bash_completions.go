...
func genBashComp(buf io.StringWriter, name string, includeDesc bool) {
	...
	WriteStringAndCheck(buf, fmt.Sprintf(`# bash completion V2 for %-36[2]s -*- shell-script -*-
...
# This function calls the %[2]s program to obtain the completion
# results and the directive.  It fills the 'out' and 'directive' vars.
__%[1]s_get_completion_results() {
    local requestComp lastParam lastChar args

    # Prepare the command to request completions for the program.
    # Calling ${words[0]} instead of directly %[2]s allows to handle aliases
    args=("${words[@]:1}")
    ...
    # When completing a flag with an = (e.g., %[2]s -n=<TAB>)
...
__%[1]s_process_completion_results() {
    local shellCompDirectiveError=%[4]d
    local shellCompDirectiveNoSpace=%[5]d
    local shellCompDirectiveNoFileComp=%[6]d
    local shellCompDirectiveFilterFileExt=%[7]d
    local shellCompDirectiveFilterDirs=%[8]d
